...
[![YouTube Tutorial](https://img.shields.io/badge/YouTube-Tutorial-red?logo=youtube)](https://www.youtube.com/playlist?list=PLaxyq6VFCJXQ)
...
By leveraging a **"Lightweight Containerization Strategy"** and a **"Virtual Panel approach"**, EVE allows clinicians to bypass complex command-line interfaces. It automates the entire workflow from raw FASTQ to a clinical-grade Excel report with a single click, focusing specifically on 26 high-priority genes associated with parathyroid disorders and an expanded panel of 413 genes related to broader endocrine pathologies.

By integrating a **"Virtual Panel"** approach, EVE filters massive genomic data using a dual-tiered strategy: a **Core Parathyroid Panel (26 genes)** for primary high-confidence screening and an **Endocrine Expansion Panel (413 genes)** for comprehensive differential diagnosis. This enables clinicians to identify pathogenic variants with high precision and minimal computational overhead.
...
* **🎯 Dual-Tiered Virtual Panel:** Implements a sophisticated filtering strategy designed to maximize diagnostic yield:
    ...
    * **Tier 2 (Expansion):** 413 genes related to broader endocrine pathologies for expanded clinical investigation and differential diagnosis
...
5.  **Variant Calling** 🔍
    ...
    * **Virtual Panel Filtering:** To optimize speed and precision, variant calling is restricted to the **26 + 413 gene intervals** with `--interval-padding 50` to capture critical splice site regions
6.  **Annotation** 📝
...
7.  **Tiered Reporting & Final Filtering** 📊
    ...
    * **Virtual Panel Filtering:** The script automatically categorizes variants into:
        ...
        * **Tier 2 Sheet:** Endocrine Expansion Panel (413 genes)
...
2. Use the provided `generate_bed.py` helper script to convert it into a BED file with exon coordinates (50 bp padding for splice sites)
...
## 🧩 Handling Variants of Uncertain Significance (VUS)

EVE uses ClinVar and gnomAD to flag and categorize variants, but the interpretation of Variants of Uncertain Significance (VUS) remains a recognized challenge in clinical genomics. Definitive reclassification ultimately requires clinical correlation, yet robust bioinformatic filtering can meaningfully reduce the interpretive burden.

We are actively working to integrate the Kaviar (Known VARiants) database and an automated ACMG/AMP-based classification module into the EVE annotation engine. This planned update will let users cross-reference variants against broader multi-ethnic allele frequency datasets, providing a more rigorous filtering strategy to distinguish common polymorphisms from genuinely uncertain variants.
...
- A genuinely empty Tier 1 sheet is biologically possible (the patient may have no variants in the 26 core parathyroid genes). The Tier 2 sheet (413 genes) should typically contain variants.
- **Custom panel users:** Double-check that your gene symbols are correct (HGNC official symbols), and look at the `Failed lookups` list printed by `generate_bed.py`.
...
If your problem is not listed here, please [open an issue on GitHub](https://github.com/hanyunseo01/EVE/issues) with:
